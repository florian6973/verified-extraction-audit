...
def compute_ll(cfg, df_all_path, base=False, extra=False):
    ...
    _base_1b = getattr(cfg, "base_model_1B", None) or os.environ.get("BASE_MODEL_1B", os.path.join(REPO_ROOT, "models", "base", "Llama_3.2-1B"))
    _base_8b = getattr(cfg, "base_model_8B", None) or os.environ.get("BASE_MODEL_8B", os.path.join(REPO_ROOT, "models", "base", "Llama_3.1-8B"))
    if base == 'scratch':

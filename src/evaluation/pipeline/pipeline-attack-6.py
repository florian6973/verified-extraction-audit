...
from src.evaluation.pipeline.experimental.config_helper import format_path, get_output_dir, get_src_ll_file
...
parser.add_argument('--exclude-prefix', action='store_true', 
                    help='Use prefix exclusion CSV (exclude prefix values) instead of default (exclude other sequences)')
...
# Load exclusion CSV and remove matching entries
# Construct exclusion CSV path based on config (same pattern as check_prefix_overlap.py)
# The exclusion CSV is in the same directory as the ll_all_output CSV files
csv_dir = os.path.dirname(CSV_PATH)
# The exclusion CSV follows the pattern: ll_all_output_False_{model}_{dataset_size}_batch_exclude_values.csv
# or ll_all_output_False_{model}_{dataset_size}_batch_exclude_prefix_values.csv if --exclude-prefix is used
if args.exclude_prefix:
    exclusion_csv_filename = f"ll_all_output_False_{model}_{dataset_size}_batch_exclude_prefix_values.csv"
else:
    exclusion_csv_filename = f"ll_all_output_False_{model}_{dataset_size}_batch_exclude_values.csv"

exclusion_csv_path = os.path.join(
    os.path.dirname(get_src_ll_file(config)),
    exclusion_csv_filename
)

if os.path.exists(exclusion_csv_path):
    assert pii_rate == 0.1, "Exclusion CSV is only for pii_rate=0.1, otherwise update the exclusion CSV"
    assert n_epochs == 3, "Exclusion CSV is only for n_epochs=3, otherwise update the exclusion CSV"
    assert model == '1B', "Exclusion CSV is only for model=1B, otherwise update the exclusion CSV"
    assert dataset_size == 10, "Exclusion CSV is only for dataset_size=10, otherwise update the exclusion CSV"
    mode_str = "prefix exclusion" if args.exclude_prefix else "other sequences exclusion"
    print(f"Loading exclusion CSV ({mode_str}) from: {exclusion_csv_path}")
    exclude_df = pd.read_csv(exclusion_csv_path)
    if 'value' in exclude_df.columns and 'value' in df.columns:
        values_to_exclude = set(exclude_df['value'].dropna().unique())
        initial_count = len(df)
        df = df[~df['value'].isin(values_to_exclude)]
        removed_count = initial_count - len(df)
        print(f"Removed {removed_count} entries that match exclusion list ({len(values_to_exclude)} unique values to exclude)")
        print(f"Dataframe size: {initial_count} -> {len(df)}")
    else:
        if 'value' not in exclude_df.columns:
            print(f"Warning: 'value' column not found in exclusion CSV: {exclusion_csv_path}")
        if 'value' not in df.columns:
            print(f"Warning: 'value' column not found in main dataframe")
else:
    print(f"Exclusion CSV not found at: {exclusion_csv_path}")
